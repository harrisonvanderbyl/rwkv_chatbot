...
loadFile = inquirer.prompt(questions)["file"][:-5]

embed = int(loadFile.split("-")[2])
layers = int(loadFile.split("-")[1])
floatmode = int(loadFile.split("-")[3])
...
def empty_state():
    state = torch.zeros(
        layers * 5, embed, dtype=floatmode)
    for i in range(layers):
        state[5*i+4] -= 1e30
    return state


tokens = loadContext(model, ctx=[], newctx=ctx1, statex=empty_state())
