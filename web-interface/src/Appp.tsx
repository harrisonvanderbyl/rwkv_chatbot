...
function App() {
  ...
  const [server, setServer] = useState("https://localhost:3001/");
  ...
  const getMessage = async (message: string, state: RwkvState) => {
    ...
    const datajson: { text: string; state: RwkvState } = await data.json();

    setState(datajson.state);
    setMessages([
      ...messages,
      { text: datajson.text, side: MessageSide.Right, icon: crow },
    ]);
  };
